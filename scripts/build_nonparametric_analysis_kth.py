#!/usr/bin/env python3
"""Build the SCI2S nonparametric-test inputs, extended with KTH-TIPS2-b.

This is `build_nonparametric_analysis.py` generalized in exactly one way: the
"Homogenea" column no longer hardcodes the winning family name
(`best_homogeneous_self_supervised`), because KTH-TIPS2-b's winning family is
`transformer`, not `self_supervised`. Instead, any `best_homogeneous_<family>`
method value is renamed to the generic label `homogeneous_family` before
pivoting -- this is safe because each (dataset, classifier) has exactly one
such row (one family wins consistently across all outer folds), which was
verified true for DTD/FMD/CUReT/Outex13 (self_supervised) and for KTH-TIPS2-b
(transformer). Everything else -- sources, pivoting, Friedman validation -- is
unchanged. Writing to a separate output directory lets the original
`results/confirmatory/nonparametric/` be diffed against this one before
anything canonical is replaced.
"""
...
OUT = ROOT / "results" / "confirmatory" / "nonparametric_with_kth"
...
SOURCES = {
    ...
    "kth_summary": ROOT / "results/extensions/kth_tips2b/nested_summary.csv",
    "kth_topk": ROOT
    / "results/extensions/kth_tips2b/topk_individual_control/nested_fold_results.csv",
}

DATASET_RELABEL = {"Outex13Official1360": "Outex", "KTHTIPS2b": "KTH-TIPS2-b"}
...
def _generalize_homogeneous(frame: pd.DataFrame) -> pd.DataFrame:
    frame = frame.copy()
    is_homogeneous = frame["method"].str.startswith("best_homogeneous_")
    per_condition_families = (
        frame.loc[is_homogeneous].groupby(["dataset", "classifier"])["method"].nunique()
    )
    if (per_condition_families != 1).any():
        raise RuntimeError(
            "expected exactly one winning homogeneous family per dataset/classifier: "
            f"{per_condition_families[per_condition_families != 1]}"
        )
    frame.loc[is_homogeneous, "method"] = "homogeneous_family"
    return frame
...
def load_matrix() -> pd.DataFrame:
    summaries = pd.concat(
        [
            ...
            pd.read_csv(SOURCES["kth_summary"]),
        ],
        ignore_index=True,
    )
    summaries["dataset"] = summaries["dataset"].replace(DATASET_RELABEL)
    summaries = _generalize_homogeneous(summaries)

    topk = pd.concat(
        [
            pd.read_csv(SOURCES["confirmatory_topk"]),
            pd.read_csv(SOURCES["outex_topk"]),
            pd.read_csv(SOURCES["kth_topk"]),
        ],
        ignore_index=True,
    )
    topk["dataset"] = topk["dataset"].replace(DATASET_RELABEL)
    topk_mean = (
